...
myApp.factory('DataService', function() {
	...
	factory.doAction = function(cmd, param, callback) {
		...
		//console.log(param);
		//console.log('-------------------')
		// alert("SN " + SN + " action " + cmd + " parameters " +
				...
				[
						...
						function($scope, DataService) {
														...
														if (basic["status"] == "online") // 设备在线
														{
															if ("extend" in result) {
																...
																if ("timestamp" in obj) // 时间戳
																{
																	...
																	var lastUpdated = localStorage
																			.getItem("lastUpdated");

																	console
																			.log("getDevice lastUpdated:"
																					+ lastUpdated)
																	if (time > lastUpdated) {
																		localStorage
																				.setItem(
							...
							function doActionReturnCallback(params) {
								if ("0" == params.errCode) {
									console
											.log("doAction return callback success");
									console.log(params);
									if ("result" in params) {
										...
										if ("timestamp" in result) // 时间戳
										{
											...
											console.log("doAction lastUpdated:"
													+ lastUpdated)
											if (time > lastUpdated) {
									...
									console
											.log("doAction return callback faild");
									console.log(params);
								}
							}
							...
							$scope.openClose = function() {
								...
								if ($scope.open == false) // 关机
								{
									...
									DataService.doAction(cmd, param, function(
											data) {
							...
							$scope.modeSet = function() {
								...
								cmd = "stopSleepMode";
								param = {};
								DataService.doAction(cmd, param,
										function(data) {
											$scope.$apply(function() {
												$scope.sleepModel = false;
												doActionReturnCallback(data);
							...
							$scope.sleepCli = function() {
								...
								{
									if (mod == "cooling" || mod == "heating"
											|| mod == "dehumidification") // 制冷,制热模式下开启睡眠模式
									{
										cmd = "startSleepMode";
										DataService
												.doAction(
										...
										$scope.sleepModel = false;
										toat("LIVEHOME_AIRCON_CONTROL_ALERT_NO_SLEEP_MODE_SET_IN_BLAST");// 送风模式下
																											// 不能开启睡眠
									} else if (mod == "auto") {
										$scope.sleepModel = false;
										toat("LIVEHOME_AIRCON_CONTROL_ALERT_NO_SLEEP_MODE_SET_IN_AUTO");// 自动模式下
																										// 不能开启睡眠
									} else {
										$scope.sleepModel = false;
							...
							$scope.gaoxiaoCli = function() {
								...
								if ($scope.gaoXiao == true) {
									if ($scope.choice == "cooling") // 在制冷模式下
																	// 开启强力
									{
										cmd = "fastCool";
										DataService
											...
											function() {
														...
														&& windV == 100) // 送风模式
												{
													...
													toat("LIVEHOME_AIRCON_CONTROL_ALERT_NO_AUTO_WIND_SPEED_SET_IN_BLAST");// 送风模式，不能设置自动风
														...
														&& (windV == 0
																|| windV == 25
																|| windV == 50 || windV == 75)) // 除湿模式，只能操作自动风
												{
													$scope.$apply(function() {
														$scope.windValue = 100;
													});
														...
														&& $scope.choice != "auto") // 静音风
												{
													...
													DataService
														...
														&& $scope.choice != "dehumidification") // 低风
												{
													cmd = "config";
													param = {
														"windSpeed" : "slow"
													...
													DataService
														...
														&& $scope.choice != "dehumidification") // 中风
												{
													cmd = "config";
													param = {
														"windSpeed" : "medium"
													...
													DataService
														...
														&& $scope.choice != "dehumidification") // 强风
												{
													cmd = "config";
													param = {
														"windSpeed" : "fast"
