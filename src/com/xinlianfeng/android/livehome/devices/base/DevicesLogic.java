...
import com.xinlianfeng.android.livehome.devices.wificontrol.BestProtocol;
...
public class DevicesLogic {
	...
	public String getVersionNumberOfSoLibrary() {
		return /*ProtocolNative.protocolGetVersion()*/"000"; // so库的版本号
	}
...
//		LogUtils.d(TAG_JSON + "json_f4f5 : " + strBuf);
		...
		tmpBuf = /*ProtocolNative.jsonCreatePassThrough(packetSequence++, 1, strBuf)*/"buf";
		if (tmpBuf != null)
...
//		LogUtils.d(TAG_JSON + "json_f4f5 : " + strBuf);
	...
	public String jsonBuildFrame(String jsonStr, int frameSeq, int deviceSubAddr) {
		String frame = /*ProtocolNative.jsonBuildFrame(jsonStr, frameSeq, DEVICE_TYPE, deviceSubAddr, 0xfe, 1)*/"frame";
		return frame;
	}
	...
	public byte[] jsonBuildhead(String jsonStr, int enctype, int cmdtype) {
		return /*ProtocolNative.jsonBuildhead(jsonStr, XLF_HEAD_MAGIC, enctype, cmdtype)*/new byte[10];
	}
	...
	public String parseByteToJSON(byte[] recv) {
		...
		String Cmd = "";
		String Result = "";
		parseString = /*ProtocolNative.bytesParseFrame(recv, DEVICE_TYPE)*/"parseString";
...
//		LogUtils.d(TAG + TAG_STATUS_REFRESH_TIME, "intervalTimeOfStatusRefresh = " + intervalTimeOfStatusRefresh);
...
//		LogUtils.d(TAG + "parseF4F5Result : " + ((parseF4F5Result == null) ? "" : parseF4F5Result));
		if (null != parseF4F5Result && !parseF4F5Result.equals("")) {
